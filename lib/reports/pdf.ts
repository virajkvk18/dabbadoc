...
export function generateHealthReportPdf(params: {
  ...
  doc.text(`Dabba Health Index: ${score}/100`, 18, 50);

  doc.setFont("helvetica", "bold");
  doc.text("Receipt analysis summary", 18, 66);
  doc.setFont("helvetica", "normal");
  doc.text(
  ...
  ].forEach((line, index) => doc.text(`- ${line}`, 22, 101 + index * 7));

  doc.setFont("helvetica", "bold");
  doc.text("7-day action plan", 18, 138);
  doc.setFont("helvetica", "normal");
  [
  ...
  ].forEach((line, index) => doc.text(`${index + 1}. ${line}`, 22, 147 + index * 7));
