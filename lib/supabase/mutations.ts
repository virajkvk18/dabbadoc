...
export async function saveReceiptAnalysis(params: {
  ...
  const { error } = await supabase.from("receipt_analyses").insert({
    ...
    cost_summary: {
      ...
      receiptType: params.analysis.receiptType ?? null,
      proteinScore: params.analysis.proteinScore ?? null,
      riskLevel: params.analysis.riskLevel ?? null,
      positives: params.analysis.positives ?? [],
      concerns: params.analysis.concerns ?? [],
      itemBreakdown: params.analysis.itemBreakdown ?? [],
      restaurantItemBreakdown: params.analysis.restaurantItemBreakdown ?? [],
      nutritionInference: params.analysis.nutritionInference ?? null,
      mealBalanceSummary: params.analysis.mealBalanceSummary ?? null,
      suggestions: params.analysis.suggestions ?? [],
